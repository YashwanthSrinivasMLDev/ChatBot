...
def get_training_dataframe():
    ...
    along with the list of original intent names.
    ...
    try:
        ...
        # --- FIX: Rename the 'text' column to 'phrase' ---
        if 'text' in df_data.columns:
            df_data = df_data.rename(columns={'text': 'phrase'})
        # Ensure 'intent' column is also present, which it should be for CLINC150

        # --- FIX: Get the original intent names from the dataset features and ensure they are strings ---
        # The 'intent' column in CLINC150 is an integer, and the actual names are in dataset.features['intent'].names
        # Explicitly convert to list of strings to prevent TypeError in classification_report
        intent_names = [str(name) for name in dataset.features['intent'].names]
        ...
        print(f"\nTotal unique intent names (from dataset features): {len(intent_names)}")

        return df_data, intent_names  # Return both the DataFrame and the intent names

    except Exception as e:
        print(f"Error loading CLINC150 dataset: {e}")
        print("Please ensure you have an internet connection for the first download.")
        return pd.DataFrame({'phrase': [], 'intent': []}), []  # Return empty list for intent_names on error
